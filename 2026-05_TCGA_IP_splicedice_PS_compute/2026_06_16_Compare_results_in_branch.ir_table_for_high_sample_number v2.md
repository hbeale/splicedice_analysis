# 2026_06_16_Compare_results_in_branch.ir_table_for_high_sample_number - v2
...
```
/mnt/splicedice_ir_example_ir_table_for_high_sample_number_branch_a805ba3_2026.06.16_18.16.43/
...
```
this_base_dir=/mnt/splicedice_ir_example_ir_table_for_high_sample_number_branch_a805ba3_2026.06.16_18.16.43/
...
cat Dockerfile_ir_table_for_high_sample_number
```
...
for new code, remove --makeRSDtable \ and add  -n 8 \
...
docker run --rm \
$this_docker \
grep -n "lambda" /usr/local/lib/python3.8/site-packages/splicedice/ir_table.py
```

std out

```bash
ubuntu@hbeale-clin-validation:/mnt/splicedice_ir_example_ir_table_for_high_sample_number_branch_a805ba3_2026.06.16_18.16.43/git_code/splicedice_analysis/code$ docker run --rm \
$this_docker \
grep -n "lambda" /usr/local/lib/python3.8/site-packages/splicedice/ir_table.py
66:        exons = sorted(exons, key=lambda x: x[0])
...
(if copying; move -n argument up so it's part of the sequence, and change -o ${this_base_dir}/ to -o ${this_base_dir}/analysis/
...
```
Tue Jun 16 18:26:02 UTC 2026
Starting ir_table with 10 samples
Loading annotation...
Annotation loaded: 528735 annotated junctions. 54.4s
Gathering inclusion counts and clusters...
Loaded 10 samples and 284452 clusters. 61.1s
Collecting junctions across all samples...
getJunctions complete: 225359 junctions. 14.5s
RSD filtering complete: 94429 junctions retained. 152.6s
Junction collection and RSD filtering complete: 94429 junctions retained. 213.8s
Writing IR table...
IR calculated for 10/10 samples
IR table written. 300.1s
Done. Total runtime: 300.1s
...
after_loc=/mnt/splicedice_ir_example_archives/splicedice_ir_example_ir_table_for_high_sample_number_branch_a805ba3_2026.06.16_18.16.43/
...
94430 /mnt/splicedice_ir_example_archives/splicedice_ir_example_ir_table_for_high_sample_number_branch_a805ba3_2026.06.16_18.16.43//analysis/_intron_retention.tsv
...
are the headings in the same order? yes (probably because i forgot to use multi-threading in the new version)
...
## are the first 1000 rows different? yes
...
```bash
8,9c8,9
< chr10:100190969-100192458:-   0.000   0.143   0.000   0.059   0.040   0.053   0.029   0.023   0.037   0.000
< chr10:100190969-100193297:-   0.000   0.143   0.000   0.000   0.040   0.053   0.057   0.023   0.071   0.000
---
> chr10:100190969-100192458:-   0.000   0.143   0.000   0.059   0.040   0.053   0.029   0.021   0.037   0.000
> chr10:100190969-100193297:-   0.000   0.111   0.000   0.000   0.040   0.048   0.057   0.021   0.071   0.000
15,16c15,16
< chr10:100235765-100236849:-   0.000   0.000   0.022   0.040   0.000   0.000   0.029   0.000   0.000   0.000
< chr10:100236970-100238021:-   0.017   0.000   0.000   0.000   0.000   0.033   0.037   0.000   0.000   0.000
---
...
```
 ...
 ````
 ubuntu@hbeale-clin-validation:/mnt$ pos=chr10:100190969-100193297
 f=$after_ir
 cat $f | grep $pos | cut -f1,3
 f=$before_ir
 cat $f | grep $pos | cut -f1,3
 chr10:100190969-100193297:-     0.023
...
```bash
 grep  "chr10:100190969-100193297:-" _allClusters.tsv
chr10:100190969-100192458:-     chr10:100190969-100193297:-,chr10:100190969-100193314:-
chr10:100190969-100193297:-     chr10:100190969-100192458:-,chr10:100190969-100193314:-,chr10:100192694-100193297:-
chr10:100190969-100193314:-     chr10:100190969-100193297:-,chr10:100190969-100192458:-,chr10:100192694-100193297:-
chr10:100192694-100193297:-     chr10:100190969-100193314:-,chr10:100190969-100193297:-
```
...
`chr10:100190969-100193314:-` is not annotated, so the new branch's `getInclusionCounts` filters it out of `counts`. When calculating IR for `chr10:100190969-100193297:-`, the new branch skips that cluster member's count, giving a smaller `intronCount` denominator and therefore a higher IR value (0.023 vs 0.021).

The fix is to not filter `counts` by annotation in `getInclusionCounts`, so all cluster members are available for the denominator regardless of whether they're annotated. In the new branch's `getInclusionCounts`:



I upaded ir_table.py, and will repeat in version 3
